// import CSS
import "../../CSS/Controls/CartospIsochron/GPFisochron.css";
...
// DOM
import IsocurveDOM from "./CartospIsocurveDOM";
...
var Isocurve = class Isocurve extends Control {
    ...
    initialize (options) {
        ...
        this._IsoPanelIsochroneModeChoiceContainer = null;
        this._typologyContainer = null;
        this._typologyLocationContainer = null;
        this._typologyLayerSelectorContainer = null;
        this._typologyRideSelectorContainer = null;
        this._typologyRideTimeSelectorContainer = null;
        this._typologyStartCalculationContainer = null;
        this._typologyTagsContainer = null;

        // typologie selector variables
        this._typologyLocationSelected = null;
    ...
    _initContainer (map) {
        ...
        var isochronecounter = this._isochroneCounter = this._createMainCounterIsochroneElement();
        picto.appendChild(isochronecounter);
        ...
        // header
        var header = this._IsoPanelIsochroneModeChoiceContainer = this._createIsoPanelHeaderElement();
        panelDiv.appendChild(header);

        // isochrone Mode Selector
        var isochroneMode = this._IsoPanelHeaderContainer = this._createIsocurveModeChoiceElement();
        panelDiv.appendChild(isochroneMode);

        // typology panel
        var typologyPanel = this._typologyContainer = this._createIsoPanelTypologyElement();
        
        // typology tags container
        var typologyTagsContainer = this._typologyTagsContainer = this._createTypologyTagsElement();
        ...
        var typologyRideSelector = this._typologyRideSelectorContainer = this._createTypologyRideSelectorElement();
        typologyPanel.appendChild(typologyRideSelector);

        // Typology Ride Time selector
        var typologyRideTimeSelector = this._typologyRideTimeSelectorContainer = this._createTypologyRideTimeSelectorElement(this._typologyTimeSelected);
        typologyPanel.appendChild(typologyRideTimeSelector);

        // Typology start calcul
        var typologyStartCalculation = this._typologyStartCalculationContainer = this._createTypologyStartCalculationElement();
        typologyPanel.appendChild(typologyStartCalculation);
        
        panelDiv.appendChild(typologyPanel);
    ...
    _updateTypologyCountMessage () {
        var element = document.getElementById("GPisochronTypologyLayerSelect");
        
        if (this._typologyServicesCount == 0 ) {
            element.innerHTML = "Sélectionner le(s) service(s) public(s)";
        } else if (this._typologyServicesCount == 1 ) {
            element.innerHTML = "1 service public sélectionné";
        } else {
            element.innerHTML = this._typologyServicesCount + " services publics sélectionnés";
        }
    }

    _updateTypologySubmitState () {
        if (this._typologyLocationSelected != "" && this._typologyLocationSelected != null && this._typologyServicesCount != 0) {
            document.getElementById("GPtypologyisochronSubmit").disabled = false;
        } else {
            document.getElementById("GPtypologyisochronSubmit").disabled = true;
        }
    }
    ...
    addEventsListeners (map) {
        var self = this;
        this.eventsListeners["isochrone:add"] = function (e) {
            logger.trace(e);
        };
        this.eventsListeners["isochrone:remove"] = function (e) {
            logger.trace(e);
        };
        map.getLayers().on("add", this.eventsListeners["isochrone:add"]);
        map.getLayers().on("remove", this.eventsListeners["isochrone:remove"]);
    }
     ...
     * @private
     */
    onSelecLocationClick (e) {
        this._typologyLocationSelected = e.target.value;
     ...
     * @private
     */
    onSelecTypologyClick (e) {
        if (this._typologyServicesSelected.includes(e.target.value)) {
            this._typologyServicesSelected.splice(this._typologyServicesSelected.indexOf(e.target.value), 1);
            this._typologyServicesCount -= 1;
            this._updateTypologyCountMessage();
            this._updateTypologySubmitState();
        } else {
            this._typologyServicesSelected.push(e.target.value);
            this._typologyServicesCount += 1;
            this._updateTypologyCountMessage();
            this._updateTypologySubmitState();
        }
